	...
	glm::vec4* dL_drot)
{
	...
	dL_dmean.x = (proj[0] * m_w - proj[3]  * mul1) * dL_dmean2D[idx].x + (proj[1] * m_w - proj[3]  * mul2) * dL_dmean2D[idx].y + (proj[2]  * m_w - proj[3]  * mul3) * dL_dmean2D[idx].z;
	dL_dmean.y = (proj[4] * m_w - proj[7]  * mul1) * dL_dmean2D[idx].x + (proj[5] * m_w - proj[7]  * mul2) * dL_dmean2D[idx].y + (proj[6]  * m_w - proj[7]  * mul3) * dL_dmean2D[idx].z;
	dL_dmean.z = (proj[8] * m_w - proj[11] * mul1) * dL_dmean2D[idx].x + (proj[9] * m_w - proj[11] * mul2) * dL_dmean2D[idx].y + (proj[10] * m_w - proj[11] * mul3) * dL_dmean2D[idx].z;
	...
	float* __restrict__ dL_dcolors)
{
	...
	const float final_acc = inside ? d_weight_acc[pix_id] : 0;
	const float final_depth = inside ? depth_acc[pix_id] : 0;
	float acc = final_acc;
	// compensate depth grad with the accumulated weight
	
	// float inv_projmatrix[16];
	// InvertMatrix(inv_projmatrix, projmatrix);
	...
	if (inside)
	{
		if (acc > 0.0)
		{
			dL_dflow.x = dL_dflows[pix_id] / acc;
			dL_dflow.y = dL_dflows[H * W + pix_id] / acc;
			dL_dflow.z = dL_dflows[2 * H * W + pix_id] / acc;
			// dL_dflow.x = dL_dflows[pix_id];
			// dL_dflow.y = dL_dflows[H * W + pix_id];
			// dL_dflow.z = dL_dflows[2 * H * W + pix_id];
		}
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (dep > min_depth & alpha * T > 0.0) {
				// float dL_ddep = dL_ddepth * dchannel_dcolor;
				// float2 dxyz_ndc = { (xy.x * 2.0 + 1.0) / W - 1.0, (xy.y * 2.0 + 1.0) / H - 1.0 };
				
				// atomicAdd(&dL_dmean2D[global_id].x, dxyz_ndc.x * dL_ddep * dep / max_depth * tan_fovx * tan_fovx / 5);
				// atomicAdd(&dL_dmean2D[global_id].y, dxyz_ndc.y * dL_ddep * dep / max_depth * tan_fovy * tan_fovy / 5);r;

				// atomicAdd(&dL_dmean2D[global_id].x, dxyz_ndc.x * dL_ddep);
				// atomicAdd(&dL_dmean2D[global_id].y, dxyz_ndc.y * dL_ddep);
				atomicAdd(&dL_dmean2D[global_id].z, dL_ddepth * dchannel_dcolor);

				// atomicAdd(&dL_dmean2D[global_id].x, dxyz_ndc.x * dL_ddep);
				// atomicAdd(&dL_dmean2D[global_id].y, dxyz_ndc.y * dL_ddep);
				// atomicAdd(&dL_dmean2D[global_id].z, dL_ddep);
				
				// approximated alpha gradient
				// dL_dalpha += (final_depth - dep) * dL_ddepth * T / acc;
				dL_dalpha += (final_depth - dep) * dL_ddepth * T;
			}
			...
			// flow loss;; does not affect other gradients
			atomicAdd(&(dL_ddir[global_id * 3]), dchannel_dcolor * dL_dflow.x);
			atomicAdd(&(dL_ddir[global_id * 3 + 1]), dchannel_dcolor * dL_dflow.y);
			atomicAdd(&(dL_ddir[global_id * 3 + 2]), dchannel_dcolor * dL_dflow.z);
			
			// flow alpha gradient TODO
			// dL_dalpha += dL_dflow.x * T ;
			// dL_dalpha += dL_dflow.y * T ;
			// dL_dalpha += dL_dflow.z * T ;
			...
			// Update gradients w.r.t. 2D mean position of the Gaussian and depth
