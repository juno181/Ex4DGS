	...
	bool prefiltered)
{
	...
	float3 p_view;
	if (!in_frustum(idx, orig_points, viewmatrix, projmatrix, prefiltered, min_depth, max_depth, p_view))
		return;
...
renderCUDA(
	...
	const float* __restrict__ dir3D,
	float* __restrict__ out_depth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < 3; ch++)
				F[ch] += dir3D[collected_id[j] * CHANNELS + ch] * alpha * T;
	...
	// Mean depth:
	// Ensure depth is normalized by accumulated alpha
	if (acc == 0.0)
	{
		D = D + (1.0f - acc) * max_depth;
	}
	else
	{
		D /= acc;
	...
	// no flow if acc is too small else nomalize it
	if (acc == 0.0)
	{
		F[3] = { 0 };
	}
